...
個人用的工作記錄 app。**一個案子一顆泡泡**——大小是「這件事現在佔掉多少腦袋」，
深淺是「已經走了多遠」。

![Babos](docs/screenshot.png)

macOS ／ Windows 桌面 app。**[下載最新版](https://github.com/qiushibo-dev/work-log/releases/latest)**

## 泡泡怎麼讀

兩個編碼互相獨立，所以四種狀態一眼分得出來：
...
**大小走常態曲線**（小 → 大 → 小）。因為設計工作最耗神的是中段，收尾剩下的多半是
機械性動作，不需要再佔版面。深淺則單調遞增——如果兩個維度同方向衰減，
「剛開始」跟「快結束」在畫面上會長得一樣。

## 功能

- 進度 1–10 的節點，點一下推進，左邊泡泡即時跟著變大變深
- **工作日誌**——常駐一行輸入框，打開、打字、Enter，三個動作
- 連結（Drive／Figma／本機路徑／URL）、案件備註
- 依狀態／類型／客戶篩選，同時作用於泡泡區與列表
- 重要度五點拖曳、六欄排序、泡泡可拖曳（會互相推擠）
- 三語（英／日／繁中）、兩種配色
- 閒置後進入只剩泡泡漂浮的螢幕保護

## 幾個刻意的決定

- **資料是你電腦上的一個 JSON 檔**，不上雲、不用註冊、不用連線
- **完全離線可用**——字體也包在 app 裡，沒網路長得一模一樣
- **泡泡上沒有進度環、沒有分類色、沒有百分比標籤。** 大小與深淺承擔全部資訊，
  泡泡維持乾淨的實心圓。這些都是想過之後否決的，理由寫在 [SPEC.md](SPEC.md)
- 整個 app 只有一個地方有儀式感：推到第 10 格之後，泡泡漲大一次然後往上飄走。
  其他地方一律安靜

> **這個 app 的目的不是專案管理，是留下痕跡。** 記錄工具的死法都一樣——做得漂亮，
> 用兩週就停。差別只在「記一筆要幾秒」，所以那個常駐的一行輸入框才是整個介面最重要的元件。
...
目前 v0.1.9，**三台機器（Mac ×2、Windows ×1）都已實際安裝**，
檔案儲存在三台上都確認正常——那是唯一會弄丟資料的環節。

其餘項目（泡泡物理、螢幕保護、完成動畫）只在 macOS 上完整走過，
Windows 上尚未逐項確認。清單在 HANDOFF.md 第 8 節。
...
四個區塊，功能面的說明在最上面那幾節，這裡只記結構上要知道的事：

| 位置 | 要知道的 |
|---|---|
| 左上・氣泡區 | 每幀解算的物理，不是一次算完的排版。氣泡會互相推擠、可以抓著拖 |
| 右上・詳細 | **未選取時顯示「最後更新的案件」**，不做「請點擊氣泡」型的空狀態（SPEC 決策 3） |
| 下・列表 | 點列＝選取案件，DETAIL 才是開完整內容。兩者是不同動作 |
| 左下・側欄 | 篩選同時作用於氣泡區與列表，兩邊永遠是同一批資料的兩種呈現 |

**左邊側欄與右邊列表各自捲動，互不連動**（v0.1.9 起）。
